...
export function PosShellPage() {
  ...
  const loadContext = useCallback(async () => {
    ...
    // Only show server-authorised location (assigned branches)
  ...
  return (
    <div>
      <PosContextBar employee={session.identity} location={location} shift={shift} />
      <div className="layout-main">
        <div className="shift-panel">
          <h2>POS workspace</h2>
          <p>Checkout is not available yet. Complete shift controls below.</p>
          ...
          {phase === 'ready' && shift && (
            <div>
              <h3>Shift {shift.status}</h3>
              <p>Opening float: RM {shift.openingFloat.toFixed(2)}</p>
              {shift.status === 'open' && (
                <>
                  <button type="button" className="btn-secondary" onClick={() => void lockShift()} disabled={busy}>
                    Lock shift
                  </button>
                  <button type="button" className="btn-secondary" onClick={() => setPhase('closing')}>
                    Close shift
                  </button>
                </>
              )}
